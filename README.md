...
| <a name="input_legacy_databases"></a> [legacy\_databases](#input\_legacy\_databases) | Map of legacy databases to create as IaaS VMs. | <pre>map(object({<br/>    size            = optional(string, "Standard_D4ds_v5")<br/>    type            = optional(string, "windows")<br/>    public_ip       = optional(bool, false)<br/>    computer_name   = optional(string)<br/>    publisher_name  = optional(string)<br/>    offer           = optional(string)<br/>    sku             = optional(string)<br/>    version         = optional(string)<br/>    source_image_id = optional(string)<br/>    os_disk_size_gb = optional(number, 127)<br/>  }))</pre> | `{}` | no |
